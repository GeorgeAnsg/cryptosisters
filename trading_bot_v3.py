...
def analyze_technical(df: pd.DataFrame, row=None, prev_row=None) -> dict:
    ...
    # --- HTF (4h) context: solo informativo aqui, el filtro se aplica en el loop ---
...
def calculate_scores(technical: dict, sentiment: dict, fear_greed: dict) -> dict:
    """
    Dos scores independientes (0-100):
      bullish_total = tech_bull(0-50) + sent_bull(0-15) + fg_bull(0-10)
      bearish_total = tech_bear(0-50) + sent_bear(0-15) + fg_bear(0-10)
    Normalizado a 0-100.
    """
    ...
    raw_bull = tech_bull + sent_bull + fg_bull
    raw_bear = tech_bear + sent_bear + fg_bear

    # Normalizar (max teorico = 50+15+10 = 75)
    bullish_total = min(100, round(raw_bull * 100 / 75))
    bearish_total = min(100, round(raw_bear * 100 / 75))
...
def run_live(exchange, pair: str, timeframe: str, interval: int, risk_profile: dict,
    ...
    while True:
        try:
            ...
            scores = calculate_scores(technical, sentiment_cache, fear_greed_cache)
            atr = technical.get("details", {}).get("atr", 0)
...
def main():
    ...
    parser.add_argument("--regime", default="neutral",
                        choices=["neutral", "bull", "bear"],
                        help="Regimen de mercado: ajusta umbrales y TP segun contexto macro")
    ...
    # Risk con overrides
    risk_profile = apply_regime(RISK_PROFILES[args.risk], args.regime)
    if args.stop_loss:
