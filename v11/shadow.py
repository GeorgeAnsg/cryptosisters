"""
V11 Shadow — corre en paralelo con V11 live pero NO ejecuta órdenes.

Utiliza exactamente los mismos parámetros, pares y modelo que V11 live.
...
Variables de entorno (mismas que V11):
  ...
  SHADOW_LOG            — ruta del log JSONL (default: ./data/v11_shadow.jsonl)
  STATE_DIR             — directorio de estado (default: ./data)

Uso:
  python -m v11.shadow
  python -m v11.shadow --btc-only
"""
...
SHADOW_LOG = Path(os.getenv("SHADOW_LOG", str(STATE_DIR / "v11_shadow.jsonl")))
STATE_DIR.mkdir(parents=True, exist_ok=True)

# ── Mismos params que V11 live ─────────────────────────────────────────────────
...
def shadow_loop(pair: str, label: str):
    print(f"[{label}] V11 shadow iniciado — {pair}")
    exchange   = make_exchange()
    strategy   = make_strategy()
    slug       = f"v11_shadow_{pair.replace('/', '_').replace(':', '_')}"
    ...
    while True:
        try:
            ...
            rp     = apply_regime(V11_RISK, signal.regime)
...
def main():
    parser = argparse.ArgumentParser(description="V11 shadow — V11 + orderbook heatmap")
    ...
    print("=" * 62)
    print("  V11 Shadow — V11 live + Orderbook Heatmap Filter")
