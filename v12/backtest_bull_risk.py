"""
V12 — Bull weekend: LONG only con distintos perfiles de riesgo.

Compara variantes de "operar en bull weekend" ajustando tamaño y SL.

Run: python -m v12.backtest_bull_risk
"""
...
# bull_wknd_cfg: dict con overrides para el fin de semana en bull
#   None  → bloqueado (actual)
#   dict  → aplica estos overrides + LONG only

CONFIGS = [
    ("Régimen  (bull wknd bloqueado)",        None),
    ("Régimen+ (LONG, riesgo 5%)",            {"risk_pct": 0.05, "stop_loss_atr_mult": 2.5}),
    ("Régimen+ (LONG, riesgo 2%)",            {"risk_pct": 0.02, "stop_loss_atr_mult": 2.5}),
    ("Régimen+ (LONG, riesgo 2%, SL 1.5x)",  {"risk_pct": 0.02, "stop_loss_atr_mult": 1.5}),
    ("Régimen+ (LONG, riesgo 3%, SL 1.5x)",  {"risk_pct": 0.03, "stop_loss_atr_mult": 1.5}),
]
    ...
    for i in range(100, len(df)):
        ...
        if is_wknd:
            if signal.regime == "bull":
                if bull_wknd_cfg is not None:
                    # Habilitar LONG only con overrides de riesgo
                    rp["weekend_mode"] = "trend"
                    rp["weekend_min_score_bonus"] = 0
                    for k, v in bull_wknd_cfg.items():
                        rp[k] = v
                    signal.bear_score = 0  # bloquear SHORT
                # else: bloqueado (range+BB sin bbl/bbu → False)
            else:
                # neutral/bear weekend → siempre permisivo
                rp["weekend_mode"] = "trend"
                rp["weekend_min_score_bonus"] = 0
...
def main():
    ...
    for period_name, start, end in PERIODS:
        ...
        print(f"{'='*72}")
        print(f"  {'Config':<44} {'PnL':>8} {'Trades':>7} {'WR':>6} {'DD':>6}")
        print(f"  {'-'*44} {'-'*8} {'-'*7} {'-'*6} {'-'*6}")
        ...
        for pair, df_full in all_dfs.items():
            ...
            for label, bull_cfg in CONFIGS:
                r = run(df, pair, bull_cfg)
                t = totals[label]
                t["pnl"]    += r["pnl"]
                t["trades"] += r["trades"]
        ...
        for label, t in totals.items():
            if t["trades"] == 0:
                print(f"  {label:<44} {'—':>8} {'—':>7} {'—':>6} {'—':>6}")
                continue
            wr = t["wins"] / t["trades"] * 100
            print(f"  {label:<44} {t['pnl']:>8.0f} {t['trades']:>7} {wr:>5.1f}% {t['dd']:>5.1f}%")
