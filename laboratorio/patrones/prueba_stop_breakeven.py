"""
14-sept-2026: el usuario señalo, con razon, que los trades que llegan a
+10-14% de ganancia favorable y acaban perdiendo NO son un movimiento
"modesto" -- es una perdida grave de una ganancia real. Los intentos
anteriores de proteger ganancias (trailing por ATR activado en R,
velocidad de subida) fallaban porque trailean de forma CONTINUA y
agresiva, cortando tambien trades sanos en su respiracion normal.

Esta idea es mas suave: no trailear continuamente, solo mover el stop a
BREAKEVEN (mas un pequeño margen) UNA VEZ que el trade alcanza una
ganancia favorable de verdad (umbral en % desde la entrada). A partir de
ahi el trade sigue exactamente igual que siempre (stop original + racha
rota + caida de pendiente + tiempo maximo) salvo que el peor caso posible
ya no es una perdida completa -- como mucho, un empate. No se toca nada
mas del comportamiento normal del trade.

Barrido de UMBRAL_GANANCIA_PCT (a partir de que ganancia se activa el
breakeven) x MARGEN_BREAKEVEN_PCT (cuanto colchon sobre el precio de
entrada, para cubrir slippage/comisiones). Sobre el sistema final ya
elegido (K=1.2, R_FIJO=4.0, racha_rota+caida 30%). Ajuste SOLO en ETH
...
K_ATR_STOP = 1.2
R_FIJO = 4.0
UMBRAL_GANANCIA_PCT_GRID = [999, 0.20, 0.15, 0.12, 0.10, 0.08, 0.06, 0.04]
MARGEN_BREAKEVEN_PCT_GRID = [0.0, 0.01, 0.02]


def _simular_trade_manual(df, idx_entrada, direccion, precio_entrada, niveles, dias_maximo,
                           senal_externa, pendiente, umbral_caida_filtro, umbral_ganancia_pct, margen_breakeven_pct):
    ...
    for i in range(idx_entrada + 1, fin + 1):
        if not breakeven_activado and umbral_ganancia_pct < 999:
            if direccion == "largo":
            ...
            if ganancia_pct >= umbral_ganancia_pct:
                ...
                if direccion == "largo":
                    stop_actual = max(stop_actual, precio_entrada * (1 + margen_breakeven_pct))
                else:
                    stop_actual = min(stop_actual, precio_entrada * (1 - margen_breakeven_pct))
        ...
        if toca_stop:
            return i, stop_actual, "stop" if not breakeven_activado else "breakeven"
        if toca_objetivo:
...
def _simular(df, candidatos, atr, racha_rota_techo, racha_rota_suelo, pendiente, umbral_switch, umbral_caida,
             umbral_ganancia_pct, margen_breakeven_pct):
    close = df["close"].to_numpy()
    fechas = df["open_time"]
    ...
    def _abrir(idx, direccion, prob):
        ...
        r = _simular_trade_manual(df, idx, direccion, close[idx], niveles, DIAS_MAXIMO, senal, pendiente,
                                   umbral_caida, umbral_ganancia_pct, margen_breakeven_pct)
        if r is None:
            return None
    ...
    def _cerrar(pos, idx_cierre, precio_cierre):
        ...
        trades.append({"pnl_eur": pnl, "motivo": pos["motivo"]})
    ...
    breakevens = sum(1 for t in trades if t["motivo"] == "breakeven")
    return capital, len(trades), ganadoras, drawdown_max_pct, breakevens


def _cargar(moneda):
    df = cargar_ohlcv_lab(moneda, "1d", estrategia="prueba_stop_breakeven")
    atr = atr_absoluto(df)
    n = len(df)
...
def ajuste_eth_2024():
    print("=== Ajuste: stop a BREAKEVEN tras ganancia -- SOLO ETH 2024 ===")
    df, atr, rt, rs, pendiente = _cargar("ETHUSDT")
    cand = candidatos_por_año(df, 2024)
    cap_base, n_b, gan_b, dd_b, be_b = _simular(df, cand, atr, rt, rs, pendiente, UMBRAL_SWITCH, UMBRAL_CAIDA_FILTRO, 999, 0.0)
    print(f"  baseline (sin breakeven): {cap_base:.2f}€ -- {n_b} trades, {gan_b} ganadoras, dd {dd_b:.2f}%")
    mejor = None
    for umbral in UMBRAL_GANANCIA_PCT_GRID:
        if umbral >= 999:
            continue
        for margen in MARGEN_BREAKEVEN_PCT_GRID:
            cap, n, gan, dd, be = _simular(df, cand, atr, rt, rs, pendiente, UMBRAL_SWITCH, UMBRAL_CAIDA_FILTRO, umbral, margen)
            print(f"  umbral_ganancia={umbral:.0%}, margen={margen:.0%}: {cap:.2f}€ ({(cap/CAPITAL_INICIAL-1)*100:+.1f}%) -- "
                  f"{n} trades, {gan} ganadoras ({gan/n*100:.0f}%), {be} cerrados en breakeven, dd {dd:.2f}%")
            if mejor is None or cap > mejor[2]:
                mejor = (umbral, margen, cap)
    print(f"  -> mejor: umbral_ganancia={mejor[0]:.0%}, margen={mejor[1]:.0%} ({mejor[2]:.2f}€) vs baseline {cap_base:.2f}€")
    return mejor[0], mejor[1]


def confirmar_y_multi_anio(umbral_elegido, margen_elegido):
    print(f"\n=== Confirmacion BTC 2024 y multi-anio con umbral_ganancia={umbral_elegido:.0%}, margen={margen_elegido:.0%} (congelado) ===")
    resultados = []
    for moneda in ["ETHUSDT", "BTCUSDT"]:
        ...
        for año in AÑOS:
            ...
            cap_actual, n_a, gan_a, dd_a, be_a = _simular(df, cand, atr, rt, rs, pendiente, UMBRAL_SWITCH, UMBRAL_CAIDA_FILTRO, 999, 0.0)
            cap_nuevo, n_n, gan_n, dd_n, be_n = _simular(df, cand, atr, rt, rs, pendiente, UMBRAL_SWITCH, UMBRAL_CAIDA_FILTRO, umbral_elegido, margen_elegido)
            gana = cap_nuevo > cap_actual
            print(f"  {año}: actual {cap_actual:.2f}€ ({n_a}/{gan_a}, dd {dd_a:.2f}%) -- "
                  f"con breakeven {cap_nuevo:.2f}€ ({n_n}/{gan_n}, {be_n} en breakeven, dd {dd_n:.2f}%) -- {'GANA' if gana else 'pierde'}")
            resultados.append(gana)
    print(f"\nResultado: gana en {sum(resultados)} de {len(resultados)} combinaciones")


if __name__ == "__main__":
    umbral, margen = ajuste_eth_2024()
    confirmar_y_multi_anio(umbral, margen)
